...
const AccountButton = () => {
  ...
  return (
    <Popover open={openPopover} handler={setOpenPopover} placement="bottom-end">
      <PopoverHandler
        {...triggers}
        className="cursor-pointer inline-block relative object-cover object-center !rounded-full w-8 h-8 ml-3 rounded-lg"
      >
        <img
          alt="avatar"
          src={imgSrc}
          onError={() => setImgSrc(defaultAuatarPngAsset)}
        />
      </PopoverHandler>
        ...
        <List className="p-0">
          ...
          {!isAuthenticated && (
            <ListItem>
              ...
              <button aria-label="log out" onClick={() => loginWithRedirect()}>
                Sign in
              </button>
            </ListItem>
          )}
